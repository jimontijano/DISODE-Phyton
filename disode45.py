 ...
 #       integration_flow=4
    ...
    while integration_flow >= 0:
        ...
        # CONTROL DE FLUJO 0: Integración estándar (región continua)
            ...
            if Verbose >= 1:
                no=np.linalg.norm(Y);
                print(f"\n Enter normalintegration H, X, Y,  {H:22.17e}, {X:22.17e}, {no:22.17e}")     
            ...
            if Verbose >= 1:
                ...
                print(f"\n Exit normal integration xout, yout, xdisapprox, H, disctype, disco: {xout:22.17e} {np.linalg.norm(yout):22.17e} {xdisaprox:22.17e} {Hout:22.17e} {disctype} {int(disco)} {disco1}")   
        # CONTROL DE FLUJO 1: Localización fina de la discontinuidad hallada
        elif integration_flow == 1:
            tol = EABS + EREL * np.max(np.abs(Y))
            if Verbose >= 1:
                print(f"\n Enter FindDisc {X:22.17e} {xdisaprox:22.17e} {xdisaprox-X:22.17e}")
